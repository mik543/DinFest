...
class HelperInstaUC{
	...
	public static function getHTMLExpires($expiresAt){
		...
		if($expireDays < 0){
			$expireDays *= -1;
			$html = "<span class='unite-color-red'>".__("The token has expired ","unlimited_elements").$expireDays.__(" ago","unlimited_elements")."</span>";
		}else{
			
			$html = __("The token will expire in ","unlimited_elements").$expireDays .__(" days. Don't worry, it should auto renew.","unlimited_elements");
		}
	...
	public static function putConnectWithInstagramButton(){
		...
		$buttonText = __("Connect With Instagram", "unlimited_elements");
		...
		if(!empty($accessToken)){
			...
			<div id="uc_instagram_reconnect_message" class="instagram-reconnect-message">
				<?php _e("The instagram access token are already set up", "unlimited_elements")?>. 
				<a id="uc_button_delete_insta_data" href="javascript:void(0)" class="unite-button-secondary"> <?php _e("Clear Access Data","unlimited_elements")?></a>
			</div>
