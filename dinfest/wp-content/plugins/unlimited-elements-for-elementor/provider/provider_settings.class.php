...
class UniteCreatorSettings extends UniteCreatorSettingsWork{
	...
	protected function addWooCatsPicker($name, $value, $title, $extra){
		...
		$arrExclude = array();
		$arrExclude["exclude"] = __("Exclude","unlimited_elements");
		$arrExclude["no_exclude"] = __("Don't Exclude","unlimited_elements");
		$arrExclude = array_flip($arrExclude);
		
		$params = array();
		$params["origtype"] = UniteCreatorDialogParam::PARAM_DROPDOWN;
		
		$this->addSelect($name."_excludeuncat", $arrExclude, __("Exclude Uncategorized Category", "unlimited_elements"), $excludeUncat, $params);
	...
	protected function addPostsListPicker($name,$value,$title,$extra){
		...
		*/
		...
		if($isForWooProducts == true){
			$arrExclude["out_of_stock"] = __("Out Of Stock Products", "unlimited_elements");
		}
		...
		$params["origtype"] = UniteCreatorDialogParam::PARAM_TEXTFIELD;
		$params["placeholder"] = __("100 posts if empty","unlimited_elements");
